...
const Tarea = () => {
    ...
    const getTareas = async() => {
      ...
      const res =await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tareas/completadas/${session?.user.id_casa}`, {
        method: "GET",
        headers: {
    ...
    const eliminarTarea = async (idtarea: String) => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/tarea/${idtarea}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
    ...
    if(session?.user.tipo.includes("Padre")){
      return (
            ...
            >
              {tareas.map((tarea) => (
                <SwiperSlide key={tarea?.idtareas}>
                  ...
                  <button className='button completarTarea' onClick={() => eliminarTarea(tarea.idtareas)}>Eliminar tarea</button>
                </SwiperSlide>
              ))}
            </Swiper>
